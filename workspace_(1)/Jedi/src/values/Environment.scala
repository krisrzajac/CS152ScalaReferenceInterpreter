...
class Environment(nextEnv:Environment) extends HashMap[Identifier, Value] with Value {
	
  def put(names: List[Identifier], vals: List[Value])
  {
    if(names.length != vals.length) throw new Exception("Lists of different length")
    else 
    {
      for(i<-0 to names.length-1)
        update(names(i), vals(i))
    }
  }
  def find(id:Identifier):Value = 
  {
    if(this.contains(id)) this.get(id).get
...
object Environment
{
  ...
  {
	println()
	println("testing Environment class")
